...
func (bc *bitmapContainer) writeTo(stream io.Writer) (int, error) {
	...
	return stream.Write(buf)//go:build (386 && !appengine) || (amd64 && !appengine) || (arm && !appengine) || (arm64 && !appengine) || (ppc64le && !appengine) || (mipsle && !appengine) || (mips64le && !appengine) || (mips64p32le && !appengine) || (wasm && !appengine)
	// +build 386,!appengine amd64,!appengine arm,!appengine arm64,!appengine ppc64le,!appengine mipsle,!appengine mips64le,!appengine mips64p32le,!appengine wasm,!appengine
	...
	func byteSliceAsContainerSlice(slice []byte) (result []container) {
		...
		// make a new slice header
		bHeader := (*reflect.SliceHeader)(unsafe.Pointer(&slice))
		result = unsafe.Slice((*container)(unsafe.Pointer(bHeader.Data)), cap(slice)/containerSize)[:len(slice)/containerSize]
		// return result
		// instantiate result and use KeepAlive so data isn't unmapped.
		runtime.KeepAlive(&slice) // it is still crucial, GC can free it)
		return
	...
	func byteSliceAsBitsetSlice(slice []byte) (result []bitmapContainer) {
		...
		// make a new slice header
		bHeader := (*reflect.SliceHeader)(unsafe.Pointer(&slice))
		result = unsafe.Slice((*bitmapContainer)(unsafe.Pointer(bHeader.Data)), cap(slice)/bitsetSize)[:len(slice)/bitsetSize]
	...
	func byteSliceAsArraySlice(slice []byte) (result []arrayContainer) {
		...
		// make a new slice header
		bHeader := (*reflect.SliceHeader)(unsafe.Pointer(&slice))
		result = unsafe.Slice((*arrayContainer)(unsafe.Pointer(bHeader.Data)), cap(slice)/arraySize)[:len(slice)/arraySize]
	...
	func byteSliceAsRun16Slice(slice []byte) (result []runContainer16) {
		...
		// make a new slice header
		bHeader := (*reflect.SliceHeader)(unsafe.Pointer(&slice))
		result = unsafe.Slice((*runContainer16)(unsafe.Pointer(bHeader.Data)), cap(slice)/run16Size)[:len(slice)/run16Size]
	...
	func byteSliceAsBoolSlice(slice []byte) (result []bool) {
		...
		// make a new slice header
		bHeader := (*reflect.SliceHeader)(unsafe.Pointer(&slice))
		result = unsafe.Slice((*bool)(unsafe.Pointer(bHeader.Data)), cap(slice)/boolSize)[:len(slice)/boolSize]
