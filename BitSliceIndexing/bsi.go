 ...
 *
 * It depends upon the roaring bitmap libraries.  It is not thread safe, so
...
/*
 * Construct a new BSI.  Min/Max values are optional.  If set to 0
...
// Return a count of unique column IDs for which a value has been set.
...
// Set a value for a given columnID.
...
// Get the value at the column ID.  Second param will be false for non-existant values.
...
type action func(t *task, batch []uint32, resutsChan chan *roaring.Bitmap, wg *sync.WaitGroup)
...
/*
 * Matrix Transpose function.  Return a bitmap such that the values are represented as column IDs
...
/*
 * De-serialize a BSI.  The value at bitData[0] is the EBM.  Other indices are in least to most
...
/*
 * Return a bitmap containing the column IDs where the values are contained within the list of values
 * provided.
 */
...
/*
 * Bits that exist in the target are cleared if they are also in the found set.
 */
